...
function toJulian($date) { return $date->format('U') / daySec - 0.5 + J1970; }
function fromJulian($j)  {
    if (!is_nan($j)) {
        $dt = new DateTime("@".round(($j + 0.5 - J1970) * daySec));
        $asd = new DateTime();
        $dt->setTimezone($asd->getTimezone());
        return $dt;
    }
}
...
class SunCalc {
    ...
    private function addTime($angle, $riseName, $setName) {
        $this->times[] = array($angle, $riseName, $setName);
    }
    ...
    function getMoonPosition($date) {
        ...
        return array(
            'azimuth' => rad2deg(azimuth($H, $phi, $c->dec)),
            'altitude' => rad2deg($h),
            'distance' => $c->dist
        );
    }
